      ...
      <main className="px-4 py-6 sm:px-6 sm:py-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
          ...
          {products.map((product: Product) => (
            ...
            <div key={product.id} className="bg-white shadow-xl dark:text-stone-900 rounded p-3 sm:p-4 flex flex-col items-center hover:scale-105 transition-transform duration-200 cursor-pointer bg-[#C2C2AF] w-full">

              <Link href={`/products/${product.id}`} className="w-full">

                <div className="w-full h-[160px] sm:h-[180px] relative overflow-hidden rounded mb-3">

                  <Image src={product.thumbnail || product.images?.[0] || "/placeholder.png"} alt={product.title} fill
