...
interface CartItemWithDetails extends Product { quantity: number; }
interface SuccessProps {
  searchParams: { canceled?: string; };
}

export default function Cart({ searchParams }: SuccessProps) {
    ...
    const fetchCartProducts = async () => {
      try {
        ...
        const detailedCart = products.map(product => ({
          ...product, quantity: cart.find(item => String(item.id) === String(product.id))?.quantity || 1,
        }));
    ...
    <div className="max-w-xl mx-auto p-8 bg-[#f5f5f5]">
      ...
      {cartItems.map(item => (
        <div key={String(item.id)} className="flex items-center gap-4 border-b py-4">
          ...
          <Image src={item.thumbnail || item.images?.[0] || "/placeholder.png"} alt={item.title} width={64} height={64} className="object-cover rounded"
          />
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center gap-2 mt-2">
              <button className="px-2 bg-gray-200 cursor-pointer rounded dark:text-stone-900" onClick={() => decreaseQuantity(String(item.id))}

              >-</button>

              <span className="dark:text-stone-900">{item.quantity}</span>

              <button className="px-2 bg-gray-200 cursor-pointer rounded dark:text-stone-900"
          ...
          <button className="bg-amber-500 text-white px-3 py-1 cursor-pointer rounded hover:bg-amber-600"
            onClick={() => removeFromCart(String(item.id))}>x</button>
        </div>
      ))}
